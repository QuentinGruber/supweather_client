...
class CityCard extends React.Component<
  { emitter: any; cityId: number },
  { cityData: CityDataObject; hoverDelete: boolean }
> {
  ...
  goToCityDetails() {
    (this.props as any).history.push({
      pathname: "/detail",
      state: { cityData: this.state.cityData.data },
    });
  }
  ...
  async componentDidMount() {
    const {
      ...
      `${process.env.REACT_APP_SERVER_URL}/weather/?cityId=${this.props.cityId}`,
            ...
            <CardHeader
              action={
                <IconButton
                  aria-label="settings"
                  onClick={() =>
                    this.props.emitter.emit("removeCity", cityData.id)
                  }
                  onMouseLeave={this.toogleDeleteState}
                  onMouseEnter={this.toogleDeleteState}
                >
                  {!this.state.hoverDelete ? (
                    <DeleteIcon />
                  ) : (
                    <DeleteForeverIcon />
                  )}
            ...
            >
              <CardContent>
                <img
                  alt={"weather icon"}
                  style={{ width: "150px" }}
                  src={`${process.env.PUBLIC_URL}assets/${getAssociatedImage(
              ...
              <CardContent>
                <Typography>
                  Feels Like :{" "}
                  {this.convertToDegreCelsius(cityData.main.feels_like)}
                </Typography>
                <Typography>
