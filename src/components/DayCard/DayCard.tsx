...
export default function DayCard(props: DayCardProps) {
  const {
    ...
    sunrise,
    sunset,
    temp: { day: dayTemp, morn: mornTemp },
    feels_like: { day: dayFeelsLike, morn: mornFeelsLike },
  } = props.dailyData;
  ...
  return (
    <Card style={{ width: "300px" }}>
      ...
      <CardContent>
        <Typography>
          Morning : {convertToDegreCelsius(mornTemp)} Feels Like :{" "}
          {convertToDegreCelsius(mornFeelsLike)}
        </Typography>
        <Typography>
          Day : {convertToDegreCelsius(dayTemp)} Feels Like :{" "}
          {convertToDegreCelsius(dayFeelsLike)}
        </Typography>
        <Typography>Pressure : {pressure} hPA</Typography>
        <Typography>Humidity : {humidity} %</Typography>
        <Typography>Orientation : {deg}°</Typography>
        <Typography>Wind : {wind} km/h</Typography>
        <Typography>Sunrise : {convertUnixTohoursMinutes(sunrise)}</Typography>
        <Typography>Sunset : {convertUnixTohoursMinutes(sunset)}</Typography>
