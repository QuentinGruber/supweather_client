...
> {
  ...
  constructor(props: any) {
    ...
    this._emitter.on("removeCity", async (cityId: number) => {
      ...
      await axios.delete(
        `${process.env.REACT_APP_SERVER_URL}/user/remove_city`,
        {
          headers: {
  ...
  async fetchCities() {
    const cities = (
      await axios.get(`${process.env.REACT_APP_SERVER_URL}/user/cities`, {
        withCredentials: true,
      })
    ).data;
    this.setState({ cities: cities });
          ...
          {this.state.cities.map((city) => {
            return (
              <div className="item">
                ...
                >
                  <CityCard key={city} emitter={this._emitter} cityId={city} />
                </Suspense>
              </div>
            );
          })}
